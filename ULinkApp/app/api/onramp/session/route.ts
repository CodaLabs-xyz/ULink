  ...
  try {
    ...
    if (!apiKeyId || !apiKeySecret) {
      ...
      return NextResponse.json(
        {
          error: 'Missing CDP API credentials. Please set CDP_API_KEY_ID and CDP_API_KEY_SECRET environment variables.',
        },
        { status: 500 }
      );
    }

    console.log('CDP API Key ID format check:', {
      hasOrganizations: apiKeyId.includes('organizations/') || apiKeyId.includes('organizations\\'),
      hasApiKeys: apiKeyId.includes('/apiKeys/') || apiKeyId.includes('\\apiKeys\\'),
      keyIdLength: apiKeyId.length
    });
        ...
        return NextResponse.json(
          {
            ...
            details: 'The CDP_API_KEY_SECRET should be your EC private key. If you have just the base64 content, ensure it\'s properly formatted.',
